...
A = [0 1 0;
    H/M 0 -G/M;
    0 H/G -R/L];  % Example matrix A (system dynamics)
...
NewPoleList = [-3 -4]; %these are the poles that I moved 
% disp(EigenList)
% NewPoleList = [-1 -11 -8+3i -8-3i]; %good for phi, p, and almost r when IC = pi/4
...
% (closed-loop poles)
v = [1]';
[K] = solveKMatrix_class(A,B,NewPoleList, v);
disp(K)
